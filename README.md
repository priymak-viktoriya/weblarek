...
### Презентер
Логика приложения реализована в основном скрипте src/main.ts — здесь презентер связывает модели данных, компоненты представления и коммуникационный слой. Отдельный класс для презентера не выделяется: у приложения одна страница, поэтому достаточно набора обработчиков событий, зарегистрированных в main.ts.

#### Принципы работы презентера:

Презентер подписывается на события от моделей (products:loaded, products:selected, basket:changed, buyer:changed) и от компонентов представления (card:select, card:preview:button-click, basket:open, basket:item-remove, basket:submit, order:change, order:submit, contacts:change, contacts:submit, success:close, modal:close).

Презентер не генерирует события — он только обрабатывает их и вызывает методы моделей и представлений.

Модели данных не знают о компонентах представления. Все данные для отрисовки презентер собирает из моделей через их публичные методы и передаёт в сеттеры представлений.

Представление перерисовывается только в двух случаях: при получении события от модели об изменении данных и при открытии модального окна.

#### Основные сценарии, реализуемые презентером:

Загрузка каталога. После ответа сервера презентер сохраняет товары в модель Products. По событию products:loaded создаёт карточки каталога и передаёт их в Gallery.

Просмотр товара. По клику на карточке (card:select) презентер сохраняет товар в модель. По событию products:selected открывает Modal и рендерит в него CardPreview — с учётом того, есть ли товар в корзине и указана ли у него цена.

Работа с корзиной. По событию basket:changed презентер обновляет счётчик в Header и перерисовывает содержимое Basket. По событию basket:open открывает модальное окно с корзиной. По событию basket:item-remove удаляет товар из модели.

Оформление заказа. По событию basket:submit открывает первую форму. По событиям order:change и contacts:change сохраняет данные в модель Buyer и обновляет формы — их валидность и текст ошибок. По событию order:submit открывает вторую форму. По событию contacts:submit собирает данные заказа, отправляет их на сервер, по успеху — очищает корзину и модель покупателя, открывает Success.

Закрытие модального окна. По событиям modal:close и success:close закрывает модальное окно.

#### Зависимости презентера:

брокер событий EventEmitter;

модели Products, Basket, Buyer;

коммуникационный класс WebLarekApi (обёртка над Api);

компоненты представления: Header, Gallery, Modal, Card, CardCatalog, CardPreview, CardBasket, Basket, Form, OrderForm, ContactsForm, Success.
